...
from geoinr.input.constraints.orientation import NormalData
...
normal_file = os.path.join(args.ROOT_DIR, prog_args.datadir, prog_args.dataset, prog_args.normal_file)
normals = NormalData(normal_file)
...
units.send_to_gpu(0)
normals.transform(scalar)
normals.convert_to_torch()
normals.send_to_gpu(0)
...
grid_dist = GridPointDataDistributedSampler(grid_points, 128)
...
for epoch in range(prog_args.num_epocs):
    ...
    normal_scalar_pred, normal_scalar_coords = model(normals.coords)
    loss_normals = oloss.normal_loss_multiple_series(normal_scalar_pred, normal_scalar_coords, normals.normal_data)

    optimizer.zero_grad()
    loss_constraints = loss_h_residuals.sum() + loss_h_var.sum() + loss_units.sum() + loss_normals.sum()
    ...
    print("Epoch {:05d} | Time(s) {:.4f} | HLoss {:.4f} | HVarLoss {:.4f}"
          " | PLoss {:.4f} | ULoss {:.4f} | NormalLoss {:.4f} | Consistency {:.4f}".format(
        epoch, dur[-1], loss_h_residuals.sum().item(),
        loss_h_var.sum().item(), loss_h_pol.sum().item(), loss_units.sum().item(), loss_normals.item(),
        loss_consistency.item()))


model_losses = {"interface_residuals": loss_h_residuals.sum().item(),
                "horizon_variance": loss_h_var.sum().item(),
                "unit_above_below": loss_units.sum().item(),
                "normals": loss_normals.item(),
