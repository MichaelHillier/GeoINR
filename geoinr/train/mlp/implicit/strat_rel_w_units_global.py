...
from geoinr.input.scalers import get_data_scalar_from_coords, get_data_scalar_from_bounds, custom_scalar, \
    custom_scalar2, \
    custom_scalar3
...
from geoinr.utils.model_processing import cut_series_by_unconformities
...
# grid_data = GridData(torch.from_numpy(grid_points.coords).float())
# grid_loader = DataLoader(grid_data, batch_size=10000, shuffle=True)
grid_points.uniformly_sample_grid_for_points(5000)
...
# Create INR model
weights_file = os.path.join(args.ROOT_DIR, prog_args.datadir, prog_args.dataset, 'plane_splus_model3L_256_100b.pth')
model = SeriesMLP(3, prog_args.embed_dim, prog_args.num_hidden_layers, n_series, prog_args.concat, weights_file).to(0)
...
for epoch in range(prog_args.num_epocs):
    ...
    loss_values.append(loss_constraints_and_consistency + loss_global)
...
# loss_values = torch.stack(loss_values).detach().cpu().numpy()
# fig, ax = plt.subplots(figsize=(24, 18), dpi=1000)
# x = np.arange(prog_args.num_epocs)
# ax.plot(x, loss_values)
# ax.set_xlabel('epoch')
# ax.set_ylabel('loss')
# ax.set_ylim([0, 0.2])
# plt.show()
# plt.savefig(prog_args.model_dir + '/lossplot.png')
...
with torch.no_grad():
    for i in range(32):
        ...
        scalar_resultant, scalar_pred, unit_pred = cut_series_by_unconformities(scalar_pred, interface.series)
...
model_output = ModelOutput(prog_args, interface=interface, unit=units, grid=grid_points,
                           model_metrics=model_losses, vertical_exaggeration=100)
...
for i in range(len(kfold_i.split)):
    ...
    # Create INR model
    model_k = SeriesMLP(3, prog_args.embed_dim, prog_args.num_hidden_layers, n_series, prog_args.concat,
                        weights_file).to(0)
    ...
    with torch.no_grad():
        for i in range(32):
            ...
            scalar_resultant, scalar_pred, unit_pred = cut_series_by_unconformities(scalar_pred,
                                                                                    interface_k_train.series)
            grid_piece.set_scalar_pred(scalar_resultant.detach().cpu().numpy())
            grid_piece.set_scalar_series(scalar_pred.detach().cpu().numpy())
            grid_piece.set_unit_pred(unit_pred.detach().cpu().numpy())
