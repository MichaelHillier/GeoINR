...
coords = concat_coords_from_datasets(interface.coords, units.coords, normals.coords)
...
for epoch in range(prog_args.num_epocs):
    ...
    loss_normal = oloss.normal_loss(normal_scalar_pred, normal_scalar_coords, normals.normal_data)

    optimizer.zero_grad()
    loss_constraints = loss_h_residuals.sum() + loss_h_var.sum() + loss_units.sum() + loss_normal
    ...
    print("Epoch {:05d} | Time(s) {:.4f} | HLoss {:.4f} | HVarLoss {:.4f}"
          " | OLoss {:.4f} | ULoss {:.4f} | NLoss {:.4f} | Consistency {:.4f}".format(
        epoch, dur[-1], loss_h_residuals.sum().item(),
        loss_h_var.sum().item(), loss_normal.item(), loss_units.sum().item(), loss_norm.sum().item(),
